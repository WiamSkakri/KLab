...
def print_model_structure(model, prefix=''):
        ...
        if isinstance(module, torch.nn.Conv2d) or hasattr(module, 'algorithm'):
            ...
            if hasattr(module, 'algorithm'):
                ...
                if hasattr(module, 'weight'):
                    ...
                    # Check if suitable for Winograd (3x3 kernels)
                    if len(module.weight.shape) >= 4:
                        ...
                        print(f"  - Winograd suitable (3x3): {is_3x3}")
    ...
    print(f"  Winograd layers: {winograd_layers}")
    ...
    print(
        f"  Winograd efficiency: {(winograd_layers/ai3_layers*100):.1f}%" if ai3_layers > 0 else "No AI3 layers")
    print(f"{'='*60}\n")


def main():
    """Main function to run DenseNet121 Winograd performance testing with AI3"""
    print("="*80)
    print("DENSENET121 WINOGRAD IMPLEMENTATION WITH AI3 LIBRARY")
    print("="*80)
    ...
    if device == "cuda":
        if not torch.cuda.is_available():
            ...
            # Set up cuDNN for optimal performance with Winograd
            torch.backends.cudnn.enabled = True
            torch.backends.cudnn.benchmark = True  # Optimize for consistent input sizes
            torch.backends.cudnn.allow_tf32 = True  # Allow Winograd algorithms
    ...
    with open(layers_csv_file, 'w', newline='') as f:
        ...
        writer.writerow(['Model', 'Layer', 'Algorithm', 'Device', 'Batch_Size', 'Input_Size',
                         ...
                         'Execution_Time_ms', 'Percentage_of_Total', 'Is_Winograd_Suitable'])
    ...
    # Check device placement after AI3 conversion
    ...
    try:
        if device == "cuda":
            if ai3_conversion_successful:
                ...
                cpu_params = sum(1 for p in model.parameters()
                                 if not p.is_cuda)
                ...
                # Fallback to standard PyTorch device placement
                model = model.to(device)
                print(f"✓ Fallback: All parameters moved to {device.upper()}")
        else:
    ...
    for i, input_size in enumerate(input_sizes):
        ...
        if layer_times:
            ...
            for layer_name, avg_time in sorted_layers:
                ...
                algorithm_info = dimensions.get('algorithm', 'unknown')

                # Check if this layer uses Winograd and is suitable for it
                layer_module = model
                try:
                    ...
                    is_winograd = hasattr(
                        layer_module, 'algorithm') and layer_module.algorithm == 'winograd'

                    # Check if 3x3 kernel (Winograd suitable)
                    is_winograd_suitable = False
                    if hasattr(layer_module, 'weight') and len(layer_module.weight.shape) >= 4:
                        kernel_h, kernel_w = layer_module.weight.shape[2], layer_module.weight.shape[3]
                        is_winograd_suitable = kernel_h == 3 and kernel_w == 3

                    winograd_indicator = " [Winograd-3x3]" if is_winograd and is_winograd_suitable else " [Winograd-Other]" if is_winograd else " [Standard]"
                except:
            ...
            with open(layers_csv_file, 'a', newline='') as f:
                ...
                for layer_name, avg_time in layer_times.items():
                    ...
                    try:
                        ...
                        winograd_suitable_status = is_winograd and is_winograd_suitable
                    except:
                        winograd_suitable_status = False

                    writer.writerow([
                        model_name, layer_name, algorithm, device, batch_size, layer_input_size,
                        in_channels, out_channels, kernel_size, stride, padding,
                        avg_time, percentage, winograd_suitable_status
                    ])
        except Exception as e:
    ...
    print(f"  ✓ AI3 Winograd algorithm optimization")
    if ai3_conversion_successful:
        ai3_layer_count = sum(
            1 for name, module in model.named_modules() if hasattr(module, 'algorithm'))
        winograd_layer_count = sum(1 for name, module in model.named_modules()
                                   if hasattr(module, 'algorithm') and module.algorithm == 'winograd')
        print(f"  ✓ {ai3_layer_count} layers converted to AI3")
        print(f"  ✓ {winograd_layer_count} layers using Winograd algorithm")
    else:
        print(f"  ⚠ AI3 conversion failed - using standard PyTorch")
    print(f"  ✓ Comprehensive layer-wise performance analysis")
    print()
    print("Next steps:")
    print("  1. Analyze the CSV files to compare AI3 Winograd performance vs other algorithms")
    print("  2. Run the same test with different algorithms (direct, gemm, smm, etc.) for comparison")
    print("  3. Compare Winograd-suitable (3x3) vs other convolution performance")
    print(f"{'='*80}")
