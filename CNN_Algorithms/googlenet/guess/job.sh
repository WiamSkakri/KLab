#!/bin/bash
#SBATCH -J googlenet_guess_gpu    # Job name
#SBATCH -o googlenet_guess_gpu.out  # Output file
...
SCRATCH_DIR=$PFSDIR/googlenet_guess_gpu_${SLURM_JOB_ID}
...
echo "Running GoogleNet Guess Algorithm Python script..."
...
echo "==============="
if [ -f "GoogleNet_guess_cuda_overall.csv" ]; then
    echo "Overall results file: GoogleNet_guess_cuda_overall.csv"
    echo "Number of test cases: $(tail -n +2 GoogleNet_guess_cuda_overall.csv | wc -l)"
fi

if [ -f "GoogleNet_guess_cuda_layers.csv" ]; then
    echo "Layer-wise results file: GoogleNet_guess_cuda_layers.csv"
    echo "Number of layer measurements: $(tail -n +2 GoogleNet_guess_cuda_layers.csv | wc -l)"
fi
...
echo "==========================================="
echo "GoogleNet Guess Algorithm Job completed successfully!"
