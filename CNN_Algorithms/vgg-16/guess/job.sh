#!/bin/bash
#SBATCH -J vgg16_guess_gpu    # Job name
#SBATCH -o vgg16_guess_gpu.out  # Output file
...
SCRATCH_DIR=$PFSDIR/vgg16_guess_gpu_${SLURM_JOB_ID}
...
echo "Running VGG-16 Guess Algorithm Python script..."
...
echo "==============="
if [ -f "VGG16_guess_cuda_overall.csv" ]; then
    echo "Overall results file: VGG16_guess_cuda_overall.csv"
    echo "Number of test cases: $(tail -n +2 VGG16_guess_cuda_overall.csv | wc -l)"
fi

if [ -f "VGG16_guess_cuda_layers.csv" ]; then
    echo "Layer-wise results file: VGG16_guess_cuda_layers.csv"
    echo "Number of layer measurements: $(tail -n +2 VGG16_guess_cuda_layers.csv | wc -l)"
fi
...
echo "==========================================="
echo "VGG-16 Guess Algorithm Job completed successfully!"
