#!/bin/bash
#SBATCH -J vgg16_implicit_precomp_gemm_gpu      # Job name
#SBATCH -o vgg16_implicit_precomp_gemm_gpu.out  # Output file
...
SCRATCH_DIR=$PFSDIR/vgg16_implicit_precomp_gemm_gpu_${SLURM_JOB_ID}
...
echo "Running VGG16 Implicit Precomputed GEMM performance test..."
python python.py 2>&1 | tee python_output.log

# Check if the script executed successfully
if [ $? -eq 0 ]; then
    echo "VGG16 Implicit Precomputed GEMM script executed successfully"
else
    echo "VGG16 Implicit Precomputed GEMM script failed with exit code $?"
...
echo "VGG16 Implicit Precomputed GEMM job completed successfully. Results are in: $RESULTS_DIR"
echo "Expected output files:"
echo "  - VGG16_implicit_precomp_gemm_cuda_overall.csv"
echo "  - VGG16_implicit_precomp_gemm_cuda_layers.csv" 
