#!/bin/bash
#SBATCH -J vgg16_winograd_gpu      # Job name
#SBATCH -o vgg16_winograd_gpu.out  # Output file
...
SCRATCH_DIR=$PFSDIR/vgg16_winograd_gpu_${SLURM_JOB_ID}
...
echo "Running VGG16 Winograd Python script..."
python python.py 2>&1 | tee python_output.log

# Check if the script executed successfully
if [ $? -eq 0 ]; then
    echo "VGG16 Winograd Python script executed successfully"
else
    echo "VGG16 Winograd Python script failed with exit code $?"
...
echo "VGG16 Winograd job completed successfully. Results are in: $RESULTS_DIR" 