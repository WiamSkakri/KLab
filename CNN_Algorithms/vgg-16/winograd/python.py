...
def main():
    ...
    for input_size in input_sizes:
        ...
        for layer_name, avg_time in sorted(layer_times.items(), key=lambda x: x[1], reverse=True):
            ...
            print(
                f"{layer_name}: {avg_time:.2f} ms ({percentage:.2f}% of total){winograd_indicator}")
    ...
    timer.remove_hooks()
    print(
        f"\nVGG16 Winograd testing completed successfully. Results saved to:\n- {overall_csv_file}\n- {layers_csv_file}")
    print(
        f"Note: {conversion_count}/{total_conv_count} layers were suitable for Winograd optimization")
