...
print_with_timestamp("Starting L40s GPU Neural Network Training")
...
if torch.cuda.is_available():
    ...
    torch.backends.cudnn.benchmark = True  # Enable for L40s optimization

# File path handling for L40s HPC
csv_file = 'combined_l40s.csv'
...
print_with_timestamp(f"Loading L40s data from {csv_file}")
df = pd.read_csv(csv_file)
print_with_timestamp(f"L40s data loaded successfully. Shape: {df.shape}")
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X), 1):
    ...
    # L40s-optimized data loaders (larger batch sizes for 48GB VRAM)
...
# L40s-optimized model class (larger architecture for 48GB VRAM)
...
class CNNExecutionPredictor(nn.Module):
    def __init__(self, input_size, dropout_rate=0.3):
        ...
        # L40s-optimized architecture with full dimensions for 48GB VRAM
        self.layer1 = nn.Linear(input_size, 2048)  # Full capacity for L40s
        self.batch_norm1 = nn.BatchNorm1d(2048)
        self.layer2 = nn.Linear(2048, 1024)        # Full capacity
        self.batch_norm2 = nn.BatchNorm1d(1024)
        self.layer3 = nn.Linear(1024, 512)         # Full capacity
        self.batch_norm3 = nn.BatchNorm1d(512)
        self.layer4 = nn.Linear(512, 256)          # Full capacity
        self.batch_norm4 = nn.BatchNorm1d(256)
        self.layer5 = nn.Linear(256, 128)          # Full capacity
        self.batch_norm5 = nn.BatchNorm1d(128)
        self.layer6 = nn.Linear(128, 64)           # Full capacity
...
# L40s-optimized training parameters
min_epochs = 200      # Original for comprehensive training
epochs = 600          # Full epoch count for L40s
patience = 150        # Original patience for thorough training
input_size = X.shape[1]
learning_rate = 0.0005  # Original learning rate
...
print_with_timestamp(
    f"L40s optimized: Epochs: {epochs}, Early Stopping Patience: {patience}")
...
print_with_timestamp("\n" + "=" * 80)
print_with_timestamp("📊 L40s NEURAL NETWORK CROSS VALIDATION SUMMARY")
print_with_timestamp("=" * 80)
...
print_with_timestamp(f"Backend Used: PyTorch (L40s GPU)")
...
print_with_timestamp("Creating L40s neural network evaluation plots...")
...
plt.ylabel('L1 Loss')
plt.title(
    f'Training vs Validation Loss (L40s - Best Fold {best_fold["fold"]})')
plt.legend()
plt.grid(True, alpha=0.3)

# Prediction vs Actual scatter plot
plt.subplot(2, 3, 2)
plt.scatter(all_actuals, all_predictions, alpha=0.6, s=10, color='darkgreen')
...
plt.title('L40s Predictions vs Actual Values')
plt.legend()
plt.grid(True, alpha=0.3)
...
plt.xlabel('Epoch')
plt.ylabel('MAPE (%)')
plt.title(f'MAPE Over Epochs (L40s - Best Fold {best_fold["fold"]})')
plt.legend()
plt.grid(True, alpha=0.3)
...
plt.title('L40s Cross-Validation Results')
...
plt.title('L40s Error Distribution')
plt.grid(True, alpha=0.3)
...
plt.axis('off')
summary_text = f"""
L40s NEURAL NETWORK SUMMARY
{'='*28}

🚀 Hardware: RTX L40s (48GB)
📊 Architecture: 6-layer DNN
   Input → 2048 → 1024 → 512 → 256 → 128 → 64 → 1
...
plt.tight_layout()
plt.savefig('nn_l40s_training_evaluation.png', dpi=300, bbox_inches='tight')
plt.close()
print_with_timestamp(
    "L40s evaluation plot saved to nn_l40s_training_evaluation.png")
...
plt.xlabel('Epoch')
plt.ylabel('R² Score')
plt.title(f'R² Over Epochs (L40s - Best Fold {best_fold["fold"]})')
plt.legend()
plt.grid(True, alpha=0.3)
...
plt.title(f'Learning Rate Schedule (L40s - Best Fold {best_fold["fold"]})')
plt.grid(True, alpha=0.3)
plt.yscale('log')
...
plt.title('L40s Training Time by Fold')
plt.grid(True, alpha=0.3, axis='y')
...
plt.title('L40s Epochs Trained by Fold')
plt.grid(True, alpha=0.3, axis='y')

plt.tight_layout()
plt.savefig('nn_l40s_detailed_metrics.png', dpi=300, bbox_inches='tight')
plt.close()
print_with_timestamp(
    "L40s detailed metrics plot saved to nn_l40s_detailed_metrics.png")

# Print summary of generated plots
print_with_timestamp("\n" + "=" * 80)
print_with_timestamp("📊 L40s VISUALIZATION SUMMARY")
print_with_timestamp("=" * 80)
print_with_timestamp("Generated L40s neural network evaluation plots:")
print_with_timestamp(
    "  1. nn_l40s_training_evaluation.png - Main L40s evaluation dashboard")
print_with_timestamp(
    "  2. nn_l40s_detailed_metrics.png - Detailed L40s metrics analysis")

# GPU memory cleanup
if torch.cuda.is_available():
    torch.cuda.empty_cache()
    print_with_timestamp("L40s GPU memory cleared")

print_with_timestamp("L40s Neural Network training completed successfully!")
