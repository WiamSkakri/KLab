#!/bin/bash
#SBATCH -J nn_prediction_training_l40s   # Job name with L40s GPU designation
#SBATCH -o nn_l40s_training.out          # Output file
...
#SBATCH --mem=48GB                       # 48GB memory (L40s has 48GB VRAM)
...
echo "============================================="
echo "L40s GPU Neural Network Training Job Started"
echo "============================================="
...
# Verify GPU allocation (optimized for L40s)
echo -e "\nVerifying L40s GPU allocation..."
...
if [[ "$GPU_NAME" == *"L40S"* ]] || [[ "$GPU_NAME" == *"L40s"* ]]; then
    ...
    echo "⚠️  Note: Expected L40s GPU but found: $GPU_NAME"
...
# Verify required packages for L40s neural network training
echo -e "\nChecking L40s neural network packages..."
...
echo -e "\n========================================="
echo "L40s Neural Network Optimization Settings"
echo "========================================="
...
# PyTorch optimizations for L40s (Ada Lovelace architecture)
export TORCH_CUDNN_V8_API_ENABLED=1
export TORCH_ALLOW_TF32_CUBLAS_OVERRIDE=1  # Enable TF32 for L40s
export TORCH_BACKENDS_CUDNN_BENCHMARK=1   # Enable cudnn benchmark for L40s

echo "✅ Environment variables set for L40s neural network optimization"
echo "CUDA_VISIBLE_DEVICES: $CUDA_VISIBLE_DEVICES"
echo "OMP_NUM_THREADS: $OMP_NUM_THREADS"

# Create timestamp for unique results directory
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
RESULTS_DIR=$SLURM_SUBMIT_DIR/results_l40s_nn_${TIMESTAMP}
mkdir -p $RESULTS_DIR

# Create a directory in scratch for the job
SCRATCH_DIR=$PFSDIR/nn_l40s_training_gpu_${SLURM_JOB_ID}
...
echo -e "\n========================================="
echo "Starting L40s GPU Neural Network Training"
echo "========================================="
echo "Results directory: $RESULTS_DIR"
echo "Scratch directory: $SCRATCH_DIR"
echo "Script: python.py"
echo "Data: combined_l40s.csv"
echo "Hardware: L40s GPU (48GB VRAM)"
echo "Algorithm: Deep Neural Network"
echo "Start time: $(date)"

# Run the L40s GPU neural network training script with comprehensive logging
echo -e "\n🚀 Executing L40s GPU neural network training..."
python python.py 2>&1 | tee training_output.log

TRAINING_EXIT_CODE=$?

echo -e "\n========================================="
echo "L40s GPU Neural Network Training Completion"
echo "========================================="
echo "End time: $(date)"
echo "Training exit code: $TRAINING_EXIT_CODE"

if [ $TRAINING_EXIT_CODE -eq 0 ]; then
    echo "✅ L40s GPU neural network training completed successfully!"
    ...
    if [ -f "training_results.csv" ]; then
        echo -e "\n📊 L40s Neural Network Training Summary:"
        python -c "
import pandas as pd
try:
...
" 2>/dev/null || echo "Could not display performance summary"
    ...
    echo "❌ L40s GPU neural network training failed with exit code: $TRAINING_EXIT_CODE"
...
L40s GPU-Accelerated Neural Network Training Job Summary
=======================================================
Job ID: $SLURM_JOB_ID
Start Time: $(date)
...
# Final summary
echo -e "\n========================================="
echo "L40s GPU Neural Network Job Summary"
echo "========================================="
echo "Job ID: $SLURM_JOB_ID"
echo "Hardware: L40s GPU (48GB VRAM)"
echo "Algorithm: Deep Neural Network"
