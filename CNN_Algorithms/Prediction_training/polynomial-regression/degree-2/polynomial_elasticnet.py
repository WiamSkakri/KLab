...
print_with_timestamp(f"🔢 TRAINING ELASTICNET POLYNOMIAL REGRESSION")
print_with_timestamp(f"{'='*60}")

# Loop through each fold
for fold, (train_idx, val_idx) in enumerate(kf.split(X), 1):
    print_with_timestamp(f"\n🔢 ELASTICNET - FOLD {fold}")
    ...
    # Print fold results
    print_with_timestamp(f"🔢 ELASTICNET Fold {fold} Results:")
...
print_with_timestamp(f"\n{'='*80}")
print_with_timestamp("🔢 ELASTICNET POLYNOMIAL REGRESSION SUMMARY")
...
print_with_timestamp(
    "✅ Visualization saved: elasticnet_polynomial_results.png")

print_with_timestamp(
    f"\n🎉 ElasticNet Polynomial Regression Training Complete!")
print_with_timestamp(
...
print_with_timestamp(f"\n📁 Generated Files:")
print_with_timestamp(f"  • best_elasticnet_model.joblib - Best trained model")
print_with_timestamp(f"  • elasticnet_training_results.csv - Detailed results")
print_with_timestamp(f"  • elasticnet_polynomial_results.png - Visualization")
print_with_timestamp(
