...
print_with_timestamp(
    "Starting LASSO Polynomial Regression (Degree 3) CNN Execution Time Prediction Training")
...
print_with_timestamp(
    f"Training LASSO Polynomial Regression (Degree 3) with {k}-Fold Cross Validation")
...
# Hyperparameter grid for LASSO regression (degree 3 only)
param_grid = {
...
def create_lasso_polynomial_pipeline():
    """Create lasso polynomial regression pipeline"""
    pipeline = Pipeline([
...
print_with_timestamp(f"🔢 TRAINING LASSO POLYNOMIAL REGRESSION (DEGREE 3)")
print_with_timestamp(f"{'='*60}")

# Loop through each fold
for fold, (train_idx, val_idx) in enumerate(kf.split(X), 1):
    print_with_timestamp(f"\n🔢 LASSO (DEGREE 3) - FOLD {fold}")
    ...
    # Print fold results
    print_with_timestamp(f"🔢 LASSO (DEGREE 3) Fold {fold} Results:")
    ...
    model_filename = f'lasso_degree3_model_fold_{fold}.joblib'
...
print_with_timestamp(f"\n{'='*80}")
print_with_timestamp("🔢 LASSO POLYNOMIAL REGRESSION (DEGREE 3) SUMMARY")
...
joblib.dump(best_model, 'best_lasso_degree3_model.joblib')
print_with_timestamp(f"Best model saved as: best_lasso_degree3_model.joblib")

# Save results to CSV
results_df = pd.DataFrame(fold_results)
results_df.to_csv('lasso_degree3_training_results.csv', index=False)
print_with_timestamp(f"Results saved to: lasso_degree3_training_results.csv")

# Create visualization (shortened for space)
print_with_timestamp("📊 Creating visualization...")

fig, axes = plt.subplots(2, 2, figsize=(14, 10))
fig.suptitle('Lasso Polynomial Regression (Degree 3) Results',
...
# Plot 2: Feature selection by fold
ax = axes[0, 1]
sparsity_percentages = [100 * (1 - f['sparsity']) for f in fold_results]
alphas = [f['alpha'] for f in fold_results]

scatter = ax.scatter(folds, sparsity_percentages, c=alphas,
                     cmap='viridis', s=100, alpha=0.7)
ax.set_xlabel('Fold')
ax.set_ylabel('Features Selected (%)')
ax.set_title('Feature Selection by Fold')
ax.grid(True, alpha=0.3)
...
summary_text = f"""
Lasso Polynomial Regression Summary:
───────────────────────────────────
Model Type: Lasso Regression (L1)
Polynomial Degree: 3
...
plt.tight_layout()
plt.savefig('lasso_degree3_polynomial_results.png',
            dpi=300, bbox_inches='tight')
plt.close()

print_with_timestamp(
    "✅ Visualization saved: lasso_degree3_polynomial_results.png")

print_with_timestamp(
    f"\n🎉 Lasso Polynomial Regression (Degree 3) Training Complete!")
print_with_timestamp(
...
print_with_timestamp(f"\n📁 Generated Files:")
print_with_timestamp(
    f"  • best_lasso_degree3_model.joblib - Best trained model")
print_with_timestamp(
    f"  • lasso_degree3_training_results.csv - Detailed results")
print_with_timestamp(
    f"  • lasso_degree3_polynomial_results.png - Visualization")
print_with_timestamp(
    f"  • Individual fold models: lasso_degree3_model_fold_*.joblib")
