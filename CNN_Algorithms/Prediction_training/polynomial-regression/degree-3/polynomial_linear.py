...
print_with_timestamp(
    "Starting LINEAR Polynomial Regression (Degree 3) CNN Execution Time Prediction Training")
...
print_with_timestamp(
    f"Training LINEAR Polynomial Regression (Degree 3) with {k}-Fold Cross Validation")
...
# Hyperparameter grid for LINEAR regression (degree 3 only)
param_grid = {
...
print_with_timestamp(f"🔢 TRAINING LINEAR POLYNOMIAL REGRESSION (DEGREE 3)")
print_with_timestamp(f"{'='*60}")

# Loop through each fold
for fold, (train_idx, val_idx) in enumerate(kf.split(X), 1):
    print_with_timestamp(f"\n🔢 LINEAR (DEGREE 3) - FOLD {fold}")
    ...
    # Print fold results
    print_with_timestamp(f"🔢 LINEAR (DEGREE 3) Fold {fold} Results:")
    ...
    model_filename = f'linear_degree3_model_fold_{fold}.joblib'
...
print_with_timestamp(f"\n{'='*80}")
print_with_timestamp("🔢 LINEAR POLYNOMIAL REGRESSION (DEGREE 3) SUMMARY")
...
joblib.dump(best_model, 'best_linear_degree3_model.joblib')
print_with_timestamp(f"Best model saved as: best_linear_degree3_model.joblib")

# Save results to CSV
results_df = pd.DataFrame(fold_results)
results_df.to_csv('linear_degree3_training_results.csv', index=False)
print_with_timestamp(f"Results saved to: linear_degree3_training_results.csv")

# Create visualization
print_with_timestamp("📊 Creating visualization...")

fig, axes = plt.subplots(2, 2, figsize=(14, 10))
fig.suptitle('Linear Polynomial Regression (Degree 3) Results',
...
plt.tight_layout()
plt.savefig('linear_degree3_polynomial_results.png',
            dpi=300, bbox_inches='tight')
plt.close()

print_with_timestamp(
    "✅ Visualization saved: linear_degree3_polynomial_results.png")

print_with_timestamp(
    f"\n🎉 Linear Polynomial Regression (Degree 3) Training Complete!")
print_with_timestamp(
...
print_with_timestamp(f"\n📁 Generated Files:")
print_with_timestamp(
    f"  • best_linear_degree3_model.joblib - Best trained model")
print_with_timestamp(
    f"  • linear_degree3_training_results.csv - Detailed results")
print_with_timestamp(
    f"  • linear_degree3_polynomial_results.png - Visualization")
print_with_timestamp(
    f"  • Individual fold models: linear_degree3_model_fold_*.joblib")
