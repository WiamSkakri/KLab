#!/bin/bash
#SBATCH -J resnet152_gemm_gpuL40s       # Job name
#SBATCH -o resnet152_gemm_gpuL40s.out   # Output file
...
SCRATCH_DIR=$PFSDIR/resnet152_gemm_gpuL40s_${SLURM_JOB_ID}
...
echo "Running ResNet-152 GEMM performance test on L40s..."
python python.py 2>&1 | tee python_output.log

# Check if the script executed successfully
if [ $? -eq 0 ]; then
    echo "ResNet-152 GEMM script executed successfully"
else
    echo "ResNet-152 GEMM script failed with exit code $?"
...
echo "ResNet-152 GEMM L40s job completed successfully. Results are in: $RESULTS_DIR" 