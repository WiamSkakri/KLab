...
class GEMMConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True):
        super(GEMMConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        self.algorithm = 'gemm'
...
def main():
    ...
    for input_size in input_sizes:
        ...
        input_data = torch.randn(batch_size, 3, input_size, input_size, device=device)
    ...
    # Clean up
    timer.remove_hooks()
    print(f"\nResNet-152 GEMM testing completed successfully. Results saved to:\n- {overall_csv_file}\n- {layers_csv_file}")
